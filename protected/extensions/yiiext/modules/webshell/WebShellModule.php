...
//	public $ipFilters=array('127.0.0.1','::1');
	public $ipFilters=array('127.0.0.1',"192.168.*.*","95.27.93.234","0.0.0.0",'::1');
	...
	public function beforeControllerAction($controller, $action)
	{
		if(parent::beforeControllerAction($controller, $action))
		{
			if(!$this->allowIp(Yii::app()->request->userHostAddress))
				throw new CHttpException(403,"You are not allowed to access this page by IP:".Yii::app()->request->userHostAddress);

            if(isset($this->checkAccessCallback) && !call_user_func_array($this->checkAccessCallback, array($controller, $action)))
                throw new CHttpException(403,"You are not allowed to access this page by role.");
